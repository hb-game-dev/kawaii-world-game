Kawaii World Game mod: farming_addons
===================================
See license.txt for license information.

Extends MultiCraft default farming mod with new plants and crops using only minetest_game API (except couple of functions for e.g. block farming).

Authors of source code
----------------------
Copyright (C) 2017-2018 SaKeL
Copyright (C) 2019-2020 MultiCraft Development Team

GNU Lesser General Public License v3.0 or later

Credits:
"farming_addons" by SaKeL (LGPLv3.0+)
https://bitbucket.org/minetest_gamers/farming_addons

Authors of textures
-------------------
HB Games (CC BY-SA 4.0)
Nova_Wostra (CC BY-SA 4.0)
